...
COMMON_ARGUMENTS = {
    ...
    "output": ArgumentConfig(name="--output", help_text="Output file path"),
    "verbose": ArgumentConfig(
        name="--verbose", help_text="Verbose output", action="store_true"
    ),
    "force": ArgumentConfig(
        name="--force", help_text="Skip confirmation prompts", action="store_true"
    ),
...
COMMAND_CONFIGS = {
    "list": CommandConfig(
        ...
        arguments=[
            ...
            ArgumentConfig(
                name="list_terms", help_text="Additional terms or filters", nargs="*"
            ),
        ],
        subcommands={
            ...
            "scripts": CommandConfig(
                ...
                arguments=[
                    ArgumentConfig(
                        name="--category", help_text="Filter by script category"
                    ),
                    ArgumentConfig(
                        name="--name",
                    ...
                    ArgumentConfig(
                        name="--id", help_text="Filter by specific script ID"
                    ),
                    ArgumentConfig(
                        name="--detailed",
    ...
    "update": CommandConfig(
        ...
        arguments=[
            ...
            ArgumentConfig(
                name="csv_file", help_text="CSV file to sync from", type="file_path"
            ),
            ArgumentConfig(
                name="--dry-run",
    ...
    "create": CommandConfig(
        ...
        subcommands={
            "category": CommandConfig(
                ...
                arguments=[
                    ArgumentConfig(
                        name="name", help_text="Category name", required=True
                    ),
                    ArgumentConfig(
                        name="--priority",
            ...
            "policy": CommandConfig(
                ...
                arguments=[
                    ArgumentConfig(name="name", help_text="Policy name", required=True),
                    ArgumentConfig(name="--category", help_text="Policy category"),
                    ArgumentConfig(
                        name="--enabled",
            ...
            "search": CommandConfig(
                ...
                arguments=[
                    ArgumentConfig(
                        name="name", help_text="Search template name", required=True
                    ),
                    ArgumentConfig(
                        name="--type",
                    ...
                    ArgumentConfig(
                        name="--criteria", help_text="Search criteria (JSON format)"
                    ),
                    ArgumentConfig(
                        name="--display-fields",
            ...
            "group": CommandConfig(
                ...
                aliases=["grp", "groups"],
                arguments=[
                    ArgumentConfig(name="name", help_text="Group name", required=True),
                    ArgumentConfig(
                        name="--type",
            ...
            "profile": CommandConfig(
                ...
                arguments=[
                    ArgumentConfig(
                        name="name", help_text="Profile name", required=True
                    ),
                    ArgumentConfig(
                        name="--type",
                    ...
                    ArgumentConfig(
                        name="--description", help_text="Profile description"
                    ),
                    ArgumentConfig(name="--category", help_text="Profile category"),
                ],
            ),
            "signature": CommandConfig(
                ...
                arguments=[
                    ...
                    ArgumentConfig(
                        name="--reset",
                        help_text="Reset to default signature (admin)",
                        action="store_true",
                    ),
                ],
            ),
            "certificate": CommandConfig(
                name="certificate",
                ...
                arguments=[
                    ArgumentConfig(
                        name="csr",
                        help_text="Generate Certificate Signing Request",
                        action="store_true",
                    ),
                    ArgumentConfig(
                        name="manage",
                        help_text="Manage certificates in Jamf Pro",
                        action="store_true",
                    ),
                    ArgumentConfig(
                        name="sign",
                        help_text="Sign configuration profiles",
                        action="store_true",
                    ),
                    ArgumentConfig(
                        name="upload",
                        help_text="Upload CSR to Jamf Pro for signing",
                        action="store_true",
                    ),
                    ArgumentConfig(
                        name="--common-name",
                        help_text="Common Name (CN) for the certificate",
                    ),
                    ArgumentConfig(
                        name="--organization", help_text="Organization name"
                    ),
                    ArgumentConfig(
                        name="--key-size",
                        help_text="RSA key size (1024, 2048, 4096)",
                        default="2048",
                    ),
                    ArgumentConfig(name="--output", help_text="Output file path"),
                    ArgumentConfig(
                        name="--certificate-id",
                        help_text="Certificate ID from Jamf Pro",
                    ),
                    ArgumentConfig(
                        name="--profile", help_text="Path to mobileconfig file to sign"
                    ),
                ],
            ),
        },
