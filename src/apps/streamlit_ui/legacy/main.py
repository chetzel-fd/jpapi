    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if (
                ...
                or "not configured" in result.stderr
            ):
                st.info(
                    "💡 Run `python3 jpapi_main.py setup auth` to configure JAMF Pro credentials"
                )
            elif "not found" in result.stderr:
                st.info(
                    "💡 Make sure you're in the correct directory and jpapi_main.py exists"
                )
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("📤 Create Export"):
            export_type = (
                "advanced-searches" if data_type == "Advanced Searches" else "policies"
            )
    ...
    with col4:
        instance_prefix = (
            get_instance_prefix(environment) if EXPORT_UTILS_AVAILABLE else "unknown"
        )
        st.metric("Instance", instance_prefix)

    if df.empty:
        st.warning(
            f"⚠️ No {object_name.lower()} data available. Click 'Create Export' to generate data."
        )
        return
        ...
        if data_type == "Advanced Searches":
            ...
            with col2:
                if "Smart" in df.columns:
                    ...
                    st.metric("Smart Searches", 0)
            with col3:
                if "Smart" in df.columns:
                    static_count = len(df[df["Smart"] == False])
                    st.metric("Static Searches", static_count)
                else:
                    st.metric("Static Searches", len(df))
        else:  # Policies
            col1, col2, col3 = st.columns(3)
            with col1:
